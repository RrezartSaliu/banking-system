...
public class PercentFeeTransactionImpl implements TransactionService {
    @Autowired
    ...
    @Override
    public void executeTransaction(Long transactionId, Long bankId) throws NotEnoughFundsException, AccountNotFromBankException, InvalidBankId, InvalidTransaction {
        Bank bank = bankService.getBankById(bankId);
        PercentFeeTransaction flatFeeTransaction = (PercentFeeTransaction) this.getById(transactionId);
        if(bank.getAccounts().stream().map(Account::getAccountId).toList().contains(flatFeeTransaction.getSourceAccount().getAccountId())){
            ...
            BigDecimal feeAmount = transactionAmount.multiply(bank.getPercentFeeValue().divide(new BigDecimal(100.00)));
