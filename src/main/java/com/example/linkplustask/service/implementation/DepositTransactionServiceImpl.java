...
public class DepositTransactionServiceImpl implements TransactionService {
    ...
    @Override
    public void executeTransaction(Long transactionId, Long bankId) throws NotEnoughFundsException, InvalidTransactionType, InvalidBankId, AccountNotFromBankException, InvalidTransaction {
        ...
        if(bank.getAccounts().stream().map(Account::getAccountId).toList().contains(transaction.getSourceAccount().getAccountId())){
            accountService.updateBalanceForDepositOrWithdraw(transaction.getSourceAccount().getAccountId(),transaction.getAmount(),"deposit");
        }
        else {
